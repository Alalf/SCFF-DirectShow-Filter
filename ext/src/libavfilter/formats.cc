                                  ...
                                  enum AVMediaType type)
{
    ...
    //---------------------------------------------------------------------
    // 2012/06/30 modified by Alalf
    MERGE_FORMATS(ret, a, b, int, formats, nb_formats, AVFilterFormats, fail);
    //---------------------------------------------------------------------
...
AVFilterFormats *ff_make_format_list(const int *fmts)
{
    //---------------------------------------------------------------------
    // 2012/06/30 modified by Alalf
    MAKE_FORMAT_LIST(AVFilterFormats, int, formats, nb_formats);
    //---------------------------------------------------------------------
    while (count--)
...
{
    //---------------------------------------------------------------------
    // 2015/12/27 modified by Alalf
    MAKE_FORMAT_LIST(AVFilterChannelLayouts, uint64_t,
...
//---------------------------------------------------------------------
// 2014/06/25 modified by Alalf
#define ADD_FORMAT(f_type, f, fmt, unref_fn, type, list, nb)        \
do {                                                        \
    type *fmts;                                             \
    void *oldf = *f;                                        \
                                                            \
    if (!(*f) && !(*f = static_cast<f_type*>(av_mallocz(sizeof(**f))))) {         \
        ...
        return AVERROR(ENOMEM);                             \
                                                            ...
                                                            \
    fmts = static_cast<type*>(av_realloc_array((*f)->list, (*f)->nb + 1,       \
                            sizeof(*(*f)->list)));           \
    if (!fmts) {                                            \
        unref_fn(f);                                        \
        if (!oldf)                                          \
...
int ff_add_format(AVFilterFormats **avff, int64_t fmt)
{
    //---------------------------------------------------------------------
    // 2012/06/30 modified by Alalf
    ADD_FORMAT(AVFilterFormats, avff, fmt, ff_formats_unref, int, formats, nb_formats);
    //---------------------------------------------------------------------
    return 0;
}
...
int ff_add_channel_layout(AVFilterChannelLayouts **l, uint64_t channel_layout)
{
    ...
    //---------------------------------------------------------------------
    // 2012/06/30 modified by Alalf
    ADD_FORMAT(AVFilterChannelLayouts, l, channel_layout, ff_channel_layouts_unref, uint64_t, channel_layouts, nb_channel_layouts);
    //---------------------------------------------------------------------
    return 0;
}
...
//---------------------------------------------------------------------
// 2014/05/28 modified by Alalf
#define FORMATS_REF(f, ref_type, ref, unref_fn)                                           \
    void *tmp;                                                                  \
                                                                                \
    if (!f || !ref)                                                             \
        return AVERROR(ENOMEM);                                                 \
    ...
    f->refs = static_cast<ref_type*>(tmp);                                                              \
...
int ff_channel_layouts_ref(AVFilterChannelLayouts *f, AVFilterChannelLayouts **ref)
{
    //---------------------------------------------------------------------
    // 2012/06/30 modified by Alalf
    FORMATS_REF(f, AVFilterChannelLayouts**, ref, ff_channel_layouts_unref);
    //---------------------------------------------------------------------
}
...
int ff_formats_ref(AVFilterFormats *f, AVFilterFormats **ref)
{
    //---------------------------------------------------------------------
    // 2012/06/30 modified by Alalf
    FORMATS_REF(f, AVFilterFormats**, ref, ff_formats_unref);
    //---------------------------------------------------------------------
}
