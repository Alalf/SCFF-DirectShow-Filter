                            ...
                            int *is_packed_rgba, uint8_t rgba_map_ptr[4])
{
    ...
    *is_packed_rgba = ff_fill_rgba_map(rgba_map, pix_fmt) >= 0;
    ...
    if (*is_packed_rgba) {
        ...
        line[0] = static_cast<uint8_t*>(av_malloc_array(w, pixel_step[0]));
        //-------------------------------------------------------------
        if (!line[0])
                       ...
                       int dst_x, int dst_y, int w, int h)
{
    ...
    for (plane = 0; plane < draw->nb_planes; plane++) {
        ...
        // 2016/11/18 modified by Alalf
        // HAVE_BIGENDIAN -> AV_HAVE_BIGENDIAN
...
AVFilterFormats *ff_draw_supported_pixel_formats(unsigned flags)
{
    ...
    for (i = 0; av_pix_fmt_desc_get(static_cast<AVPixelFormat>(i)); i++)
        if (ff_draw_init(&draw, static_cast<AVPixelFormat>(i), flags) >= 0 &&
