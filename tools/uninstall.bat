...
if "%1"=="-sys" (
  ...
  rem Normal
  if "%PROCESSOR_ARCHITECTURE%"=="AMD64" (
    "tools\bin\regsvrex32.exe" /s /u "bin\Debug_Win32\scff_dsf_Win32.ax"
    "tools\bin\regsvrex64.exe" /s /u "bin\Debug_x64\scff_dsf_x64.ax"
    "tools\bin\regsvrex32.exe" /s /u "bin\Release_Win32\scff_dsf_Win32.ax"
    "tools\bin\regsvrex64.exe" /s /u "bin\Release_x64\scff_dsf_x64.ax"
  ) else (
    "tools\bin\regsvrex32.exe" /s /u "bin\Debug_Win32\scff_dsf_Win32.ax"
    "tools\bin\regsvrex32.exe" /s /u "bin\Release_Win32\scff_dsf_Win32.ax"
  )
